-- Trigger und Funktionen für das Löschen von Rollen-Einträgen in den Tabellen person_rolle_objekt_neu und koerperschaft_rolle_objekt_neu
...
RETURNS TRIGGER AS $
BEGIN
    DELETE FROM person_rolle_objekt_neu
    WHERE objekt_id = OLD.isbn AND objekt_typ = 'Buch';

    DELETE FROM koerperschaft_rolle_objekt_neu
    WHERE objekt_id = OLD.isbn AND objekt_typ = 'Buch';
...
BEGIN
    DELETE FROM person 
    WHERE person_id NOT IN (SELECT DISTINCT person_id FROM person_rolle_objekt_neu)
    ...
    -- diese ist aufgrund der 1-n-Beziehung  nicht in der person_rolle_objekt_neu gelistet
    
    DELETE FROM koerperschaft 
    WHERE koerperschaft_id NOT IN (SELECT DISTINCT koerperschaft_id FROM koerperschaft_rolle_objekt_neu)
    ...
    -- diese sind aufgrund der 1-n-Beziehungen  nicht in der koerperschaft_rolle_objekt_neu gelistet
END;
$ LANGUAGE plpgsql;
